...
function assertNoForbiddenEnv() {
  ...
  if (forbidden.length) {
    throw new Error(`Unsafe environment variables detected for local load test: ${forbidden.join(', ')}`);
  }
}
...
function assertEnv() {
  ...
  if (!LOCAL_BASE_URL_PREFIXES.some((prefix) => baseUrl.startsWith(prefix))) {
    throw new Error(`BASE_URL must be localhost/127.0.0.1 for this local load test. Received: ${baseUrl}`);
  }
